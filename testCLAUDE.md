...
[归因分析] 共 2 个参数 FAIL：
  ...
  bandgap_up：偏差 16.5%（容差 15%，属 1.10 倍）
    → 类别：precision_sensitivity（偏差 < 2 倍容差）
    → 物理解释：能隙偏差仅为容差的 1.10 倍，属精度参数收敛效应正常范围
    → insertion_note：💡 关于 bandgap_up 的参考值：本教程参考值为 0.205 eV，
      测试计算得到 0.180 eV（偏差 16.5%），属精度参数收敛效应正常范围。
```
...
1. 【result_deviation】NiO 能隙实测值与教程预期偏差 12.5%（Step 6.3）
   ...
   **After（插入归因说明后）：**
   ...
   > **💡 关于能隙参考值：** 本教程中 bandgap_up 参考值为 2.4 eV，测试计算得到 2.1 eV（偏差 12.5%）。DFT(PBE) 计算能隙存在系统性不确定性（通常 ±15%），若读者得到相近结果，说明计算流程正确；若关注精确能隙值，建议使用杂化泛函（HSE06）或 GW 方法。
   ```
