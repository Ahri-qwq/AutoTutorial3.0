...
- 让用户看到你的决策过程
- 所有回复、分析、思考过程，必须使用简体中文（代码中的变量名和注释除外）
...
**1.1a 检索结果使用规范（防归属错误）：**
当 chunk 内容涉及具名工具（abacus-plot、Atomkit、Phonopy、abacustest 等）、参数名或计算类型时：
1. 优先相信 chunk **正文**中的动词描述（"使用 xxx 命令"、"用 xxx 绘图"、"调用 xxx 函数"）
2. chunk 的来源文件名和标题路径提供**文档归属参考**，不代表正文每句话都属于标题中提到的工具
3. 若不确定归属，**必须回读 `source` 指向的原文**对应章节，不要仅凭文件名或标题路径推断

**1.1b 必须回溯原文的触发信号：**
当检索到的 chunk 出现以下任一情况时，必须用 `Read` 工具读取完整源文件（路径为 `data/knowledge_source/<source>`）：
1. 出现引用性表述："如上所述"、"参见"、"同第X章"、"见下文"
2. 同一 chunk 出现两个以上具名工具名（可能是对比章节，上下文在 chunk 之外）
3. INPUT 参数说明缺少适用计算类型（只说参数名和默认值，没说"仅在 relax 下有效"等限定）
4. chunk 明显被截断（结尾是代码块中间或列表中间）
