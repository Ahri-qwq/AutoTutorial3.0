...
**优化轨迹文件：** `OUT.Si/STRU_ION_D`（最终结构）、`OUT.Si/STRU_ION1_D`、`OUT.Si/STRU_ION2_D`……（各步轨迹）。最终结构可直接用于后续计算。

以本例（50 Ry、4×4×4 k点）为例，优化收敛后的晶格常数约为 **5.476 Å**（经 Bohrium 平台实际计算验证，Job 22316482）。PBE 泛函对 Si 存在约 0.5–0.8% 的系统性高估，实验值为 5.43 Å，DFT/PBE 计算值通常在 5.46–5.48 Å，两者在 PBE 泛函的预测精度范围内相符。

> **注意：** ecutwfc 和 k 点密度会影响最终晶格常数。本例参数（50 Ry，4×4×4）可快速收敛，但精度较低。生产计算建议适当提高（如 80 Ry，8×8×8）并做收敛性测试。
# 第六章：输出文件解读
...
> **注意：** 本节示例中的能量数值（-1.235106e+02 eV）仅为演示输出格式，实际计算结果取决于体系、赝势、参数设置等因素。
...
> **提示：** 本教程示例使用 SCF 计算，输出日志为 `running_scf.log`。其他计算类型（relax、cell-relax、md）的日志文件命名可能有所不同，请查看 `OUT.suffix/` 目录中的实际文件名。
...
> **重要提示：**
> - abacustest 功能持续更新，具体命令和参数请执行 `abacustest <command> -h` 查看最新帮助
> - 完整文档见 GitHub：https://github.com/pxlxingliang/abacus-test
...
> **注意：**
> - `--input` 参数中逗号分隔符两侧不要有空格
> - 参数名需与 ABACUS INPUT 文件中的参数名一致
...
> **提示：**
> - 具体功能和参数请查阅官方文档：https://github.com/pxlxingliang/abacus-test
> - 使用前建议执行 `abacustest <command> -h` 查看帮助信息
# 附录
