...
计算得到弹性常数后，需要判断结果是否可信。以下是三个关键检验标准：
...
**判断标准：**
- 非对角元素（如 C_14, C_15, C_16 等）应 < 0.1 GPa
- 如果非对角元素达到 0.1 GPa 以上，说明 k 点密度或 SCF 精度不足

对于 Al 的计算结果，非对角元素量级约为 10⁻¹⁰ GPa，可视为数值噪声，说明计算精度足够。

**2. 力学稳定性判据（Born 稳定性判据）**
...
对于 Al 的典型结果（C_11=114 GPa, C_12=62 GPa, C_44=32 GPa）：
- C_11 - C_12 = 114 - 62 = 52 GPa > 0 ✓
- C_11 + 2C_12 = 114 + 2×62 = 238 GPa > 0 ✓
- C_44 = 32 GPa > 0 ✓

全部满足，说明 Al FCC 结构力学稳定。

**3. Pugh 比判断脆韧性**

Pugh 比定义为体弹模量与剪切模量之比：B/G

**判断标准：**
- B/G > 1.75：材料倾向于韧性（延展性）
- B/G < 1.75：材料倾向于脆性
...
说明 Al 是韧性金属，符合实际认知（Al 具有良好的延展性和可加工性）。

**物理意义：**
- 体弹模量 B 大：抵抗体积变化能力强
- 剪切模量 G 相对较小：容易发生剪切形变
- B/G 大：材料在外力下更倾向于通过剪切滑移来释放应力，而不是脆性断裂

**对比：**
- 韧性金属（Cu, Al, Au）：B/G > 2.0
- 脆性陶瓷（Si, SiC）：B/G < 1.5
...
参见第3.4.4节"结果可信度判断"，三个关键检验：
- 对称性检查：非对角元素 < 0.1 GPa
- 力学稳定性判据：C_11-C_12>0, C_11+2C_12>0, C_44>0
- Pugh 比判断：B/G 与材料脆韧性是否一致
- 应力-应变拟合线性度：R² > 0.99
- 与实验值或文献值对比（注意 PBE 系统性偏差）

**Q5：弹性常数出现负值怎么办？**

弹性常数为负说明结构力学不稳定，可能原因：
- 结构未充分优化（残余应力过大）
- 应变幅度过大（超出线性范围，建议 ±0.5%~±1%）
- k 点或截断能不收敛（导致应力计算不准确）
- 结构本身不稳定（需要检查是否为亚稳态或过渡态）

**Q6：为什么 relax 步骤要允许原子弛豫？**

应变后的结构中，原子位置偏离新的平衡位置，会产生内应力。允许原子弛豫（relax）可以：
- 消除内应力，得到更准确的应力张量
- 符合实验条件（准静态过程）
- 这种方法称为"弛豫离子模型"，是计算弹性常数的标准做法

如果使用 scf（冻结离子），会得到"钳位离子"弹性常数，通常比弛豫离子结果偏硬。
