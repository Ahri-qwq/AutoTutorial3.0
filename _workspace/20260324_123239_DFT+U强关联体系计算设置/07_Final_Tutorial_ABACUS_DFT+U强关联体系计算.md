...
DFT+U 计算结果受多个因素影响，不同环境下复现同一体系的结果会存在一定偏差。以下是各物理量的判断标准。

**总能量**

总能量的绝对值很大（本例约 -9255 eV），其中包含了所有电子的动能和相互作用能。复现时，总能量的绝对偏差可能达到几个 eV——这在物理上是微小的，但绝对值看起来很大。合理的判断方式是**相对偏差**：

$$\text{相对偏差} = \frac{|E_{\text{计算}} - E_{\text{参考}}|}{|E_{\text{参考}}|} < 0.1\%$$

引起总能量偏差的主要原因：LCAO 基组截断（`ecutwfc`）、k 点网格密度、ABACUS 版本差异。

**能隙**

能隙对 U 值、k 点密度和 LCAO 基组大小均很敏感，是 DFT+U 计算中波动最大的量。**10–20% 的相对偏差是正常的。**

本例 spin down 能隙约 2.79 eV，与实验值（~4 eV）仍有差距——这不是计算错误，而是 DFT+U 方法本身的局限。DFT+U 相比纯 GGA 有显著改善，但通常仍低估强关联体系的能隙。

常见导致能隙偏低的原因：

- k 点网格不够密（建议收敛测试：4×4×4 → 6×6×6 → 8×8×8）
- U 值偏小（NiO 文献中 U 值通常在 3–8 eV 范围，结果对 U 较敏感）
- LCAO 基组不够大（本例 `Ni_gga_9au_100Ry_4s2p2d1f.orb` 已是较大基组）

**磁矩**

原子局域磁矩受 U 值和 LCAO 基组影响，**5–15% 的偏差属于合理范围。**

对于 NiO，实验测量的 Ni 原子局域磁矩约为 1.9 μB（Phys. Rev. B 27, 6964, 1983）。本教程案例计算结果约 1.83 μB，与实验值偏差约 4%，在合理范围内。

判断反铁磁计算是否正确的关键不在于磁矩的精确数值，而在于：

1. `total magnetism = 0`（整体无净磁矩）
2. 两个 Ni 原子磁矩**大小相等、方向相反**
3. O 原子磁矩接近零

满足以上三点，即可认为计算成功收敛到反铁磁基态。

**多解问题与局域极小值**

DFT+U 特有的风险：不同初始磁矩可能收敛到不同的 occupation matrix，对应不同的能量极小值。如果复现结果与预期差异很大（能量偏高超过 1 eV、磁序不同），大概率是收敛到了不同的局域极小值，而非计算错误。

处理方法：使用 `omc 1` 或 `omc 2` 固定初始 occupation matrix（见第五章），或用 U-Ramping 方法从 U=0 逐步增大到目标值。
