...
以下是 ELF.cube 文件头部的典型格式（来自案例一的 H₂O 计算）：
...
前两行是注释行（VESTA 不读取），从第三行开始是结构信息：

- 第 3 行：原子数（3）+ 格点原点坐标（Bohr）
- 第 4-6 行：各方向的 FFT 格点数 + 步长向量（Bohr）
- 第 7-9 行：原子信息（原子序数 价电子数 x y z，坐标单位 Bohr）
- 后续行：ELF 数据（z 变化最快，每行 6 个数据）

对于本例，FFT 格点为 180×180×180，步长 0.155556 Bohr，对应约 28 Bohr × 28 Bohr × 28 Bohr（~14.8 Å）的真空盒子。
# 三、案例一：水分子 ELF（自旋非极化）
...
文件格式说明：
- 第 3 行：3 个原子，格点原点在 (0, 0, 0)
- 第 4-6 行：180×180×180 格点，步长 0.155556 Bohr（约 0.082 Å）
- 第 7-9 行：H1、H2、O 的原子序数、价电子数、笛卡尔坐标（Bohr）
- 远离分子的区域 ELF≈0（真空中无电子）
...
LCAO 基组存在不完备性，在远离原子核的区域，动能密度的数值行为可能出现问题，导致真空区出现非物理的非零 ELF。ABACUS 采用文献 [2] 的建议，在 ELF 计算的分子部分加入 10⁻⁵ 的小量修正，在保证远场行为正确的同时不影响近核区域的结果。PW 基组没有此问题。
...
**原因：** LCAO 基组不完备，在远离原子核处，ELF 分子中的动能密度可能更快趋零，导致比值出现异常。

**处理方式：** ABACUS 已内置稳定性修正（分子加 10⁻⁵ 小量），该修正在当前版本中默认开启，无需用户手动操作。若仍发现异常，建议切换到 PW 基组进行验证。
