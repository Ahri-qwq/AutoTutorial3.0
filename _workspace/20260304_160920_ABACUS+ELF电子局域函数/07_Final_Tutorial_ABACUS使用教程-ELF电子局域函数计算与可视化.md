---
title: "ABACUS 使用教程｜电子局域函数（ELF）计算与可视化"
author: "AutoTutorial 3.0"
date: "2026-03-04"
topic: "ELF 电子局域函数 后处理 可视化"
task_type: "C"
has_case: true
word_count: ~469行
chapters: 5
...
LCAO 计算只需在 INPUT 和 STRU 中做少量修改，坐标与 PW 相同：
...
> **注意：** H 的轨道文件也存在 8au 截断半径的版本（`H_gga_8au_100Ry_2s1p.orb`）。请以 GitHub 算例中的实际文件为准。
...
初始磁矩设置为 2.3 μB（接近铁的实验磁矩 ~2.2 μB）。合理的初始磁矩对收敛到铁磁基态至关重要——过小的初始磁矩可能导致 SCF 收敛到非磁态。
...
- 自旋向上和向下的差异体现了铁磁性——自旋向上电子更多（磁矩约 2.2 μB），其 ELF 峰值略高
...
- **Bader 电荷分析**：与 ELF 互补，从积分角度定量分析原子间电荷转移，ABACUS 支持输出 Bader 分析所需的电荷密度
- **差分电荷密度**：计算成键前后电荷密度之差，可配合 ELF 共同分析化学键特征
- **OFDFT 中的 ELF**：无轨道密度泛函理论同样支持 `out_elf`，适用于数千原子量级体系的快速分析
