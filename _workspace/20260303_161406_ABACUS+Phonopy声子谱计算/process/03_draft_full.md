...
`band.conf` 和 `plot_pho.gp` 文件在后续步骤中使用，也需要提前准备好。
...
- `m 0 0 0` 表示三个方向均不固定（允许受力计算时原子受约束为0位移，但这里是在 SCF 计算原始结构时的设置）。
...
由于 FCC 的晶格对称性极高，这里**只产生 1 个微扰构型**。对称性较低的体系可能产生多个（STRU-001、STRU-002、...），每个都需要用 ABACUS 单独计算。

**扩胞大小的选取：** 本例 FCC Al 晶格常数约 4.035 Å，2×2×2 超胞的边长约为 8.07 Å，接近 10 Å 下限。对于需要更高精度的计算，可以尝试 3×3×3 扩胞，但计算量会增加 3–4 倍。
...
**验证受力输出是否正确：** 计算完成后，在 `OUT.Al-fcc/running_scf.log` 中搜索关键词 `FORCE`：

```bash
grep "FORCE" OUT.Al-fcc/running_scf.log | head -5
```

如果看到原子受力的数值输出，说明力的计算成功。

**处理多个微扰构型（非本例）：** 如果 Phonopy 产生了多个微扰构型（STRU-001、STRU-002 等），常见做法是创建子目录：

```bash
mkdir disp-001 disp-002
cp INPUT KPT psp/ disp-001/
cp INPUT KPT psp/ disp-002/
# 在 disp-001/INPUT 中设置 stru_file STRU-001
# 在 disp-002/INPUT 中设置 stru_file STRU-002
# 分别提交 ABACUS 计算
```
...
```bash
phonopy -f ./OUT.Al-fcc/running_scf.log
```

如果有多个微扰构型，需要列出所有 `running_scf.log` 的路径：
...
2. `running_scf.log` 中是否包含力输出（`grep FORCE running_scf.log`）
3. 路径是否正确
