---
title: "用 ABACUS 做周期体系 RT-TDDFT：从速度规范的困境到混合规范"
...
# 用 ABACUS 做周期体系 RT-TDDFT：从速度规范的困境到混合规范
...
**速度规范**：将电场归为矢势 $\mathbf{A}(t) \propto \int_0^t \mathbf{E}(t') dt'$，保持哈密顿量的周期性，适用于晶体。

理论上，两种规范通过如下相位变换等价：

$$\psi^{\text{velocity}}(\mathbf{r}, t) = e^{i\mathbf{A}(t)\cdot\mathbf{r}} \psi^{\text{length}}(\mathbf{r}, t)$$

在完备基组下，这个相位因子能被精确表达。但在 NAO 基组下，波函数展开为原子轨道线性组合：

$$\psi(\mathbf{r}, t) = \sum_{\mu} c_\mu(t) \phi_\mu(\mathbf{r})$$

这里的原子轨道 $\phi_\mu(\mathbf{r})$ 不会自动获得 $e^{i\mathbf{A}(t)\cdot\mathbf{r}}$ 相位——速度规范的 NAO 展开忽略了每个原子轨道内部由矢量势引起的相位变化，进而引入系统性误差。增大基组可以缓解误差，但计算成本急剧上升。**混合规范直接从根源上修正这一误差。**
...
在这个修正下，哈密顿量矩阵元和重叠矩阵均需相应修改。混合规范同时包含矢量势 $\mathbf{A}$ 和标量场，相比长度规范，其外场项保持周期性，不破坏布洛赫定理；相比速度规范，它补偿了 NAO 基组不完备带来的相位误差。

混合规范和长度规范之间满足严格的规范变换关系，理论结果完全等价。在 ABACUS 的测试中，混合规范与长度规范（非周期体系）的结果高度吻合，而速度规范则存在明显偏差。
...
```bash
# 第一步：基态 SCF
abacus   # 使用 SCF 版本的 INPUT

# 第二步：TDDFT 演化（使用新 INPUT，其他文件不变）
abacus   # 使用 TDDFT 版本的 INPUT
```

ABACUS 的 TDDFT 计算会读取 SCF 产生的 `OUT.*` 目录中的电荷密度作为初始态。

### 后处理：电流 → 介电函数

TDDFT 演化结束后，`OUT.*` 目录中会输出电流文件（请参考实际运行后目录中的文件名）。电流 $J(t)$ 与电偶极矩的时间导数成正比：$J(t) \propto \dot{D}(t)$，因此傅里叶变换时：
...
使用 ABACUS 的后处理脚本（具体脚本名称和用法请参考 `tools/plot-tools` 目录下的说明文件，或 ABACUS 官方算例仓库中的示例）：

```bash
# 参考 tools/plot-tools 目录下的说明运行后处理脚本
python tools/plot-tools/<script_name>.py \
...
td_gauss_t0      50
td_gauss_sigma   0.5
td_gauss_freq    0.372         # 激光中心频率（约 1.0 eV，请根据实际需求设置）
td_gauss_phase   0.0
```

> **参数说明**：`td_gauss_freq` 的单位请参考 ABACUS 在线文档。高次谐波谱通过对 Si 单胞施加一系列不同场强的高斯脉冲来测试。
...
实验规律清晰：随场强增大，HHG 谱中依次出现 1、3、5、7 次谐波信号（奇次谐波，源于 Si 的反演对称性），与理论预期完全吻合。这也证明混合规范在强场和弱场条件下均能给出可靠结果。

> **混合规范的计算效率优势**：相比速度规范，混合规范在处理非局域势时更高效，在相同基组下精度更高。这意味着可以使用较小的基组（如 2s2p1d）完成高精度的 HHG 计算，而速度规范为达到同等精度则需要更大的基组。
...
**原因：** 频率分辨率 $\Delta\omega \approx 1/T_{\text{total}}$，演化时间越短，分辨率越差。
...
`td_gauss_freq` 的具体单位请参考 ABACUS 在线文档（https://abacus.deepmodeling.com/en/latest/advanced/input_files/input-main.html）。对于宽带激发（线性响应），设置 `td_gauss_freq 0.0` 并配合小 sigma 即可。
