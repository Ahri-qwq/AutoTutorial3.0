...
本教程以晶态二氧化硅（β-方英石 SiO₂）为例，演示如何使用 **ABACUS** 结合 **PYATB** 计算材料的介电函数，并通过 Python 后处理脚本进一步得到折射率、消光系数、吸收系数等线性光学性质。
...
ABACUS 完成自洽计算后，通过设置 `out_mat_hs2 = 1` 和 `out_mat_r = 1`，可以将实空间矩阵 $H_{\nu\mu}(\mathbf{R})$、$S_{\nu\mu}(\mathbf{R})$ 以及位置偶极矩阵 $r_{\nu\mu,a}(\mathbf{R})$（$a=x,y,z$）以稀疏格式输出。PYATB 读取这三组矩阵，在任意 $\mathbf{k}$ 点重建哈密顿量，用于后续物性计算。
...
本教程选用**β-方英石（β-cristobalite）SiO₂** 单胞作为示例材料。β-方英石是 SiO₂ 的一种高温相，具有立方对称结构（空间群 $Fd\bar{3}m$）。SiO₂ 是典型的宽带隙绝缘体，光学透明窗口覆盖紫外到近红外波段，其光学性质在光学涂层、光纤、半导体器件等领域有重要应用价值。
...
INPUT 文件控制所有计算参数。对于 PYATB 接口，有三个参数必须打开：`out_mat_hs2`、`out_mat_r` 和 `out_chg`。
...
LATTICE_CONSTANT
1.8897261246257702          # Bohr/Å 换算因子（即 1 Å）
...
由于 SiO₂ 是各向同性立方结构，三个对角分量（xx、yy、zz）完全重合，非对角分量为零。实部 $\epsilon_1(0) \approx 1.90$（对应折射率 $n(0) \approx 1.38$），虚部 $\epsilon_2$ 在约 10 eV 以下（带隙以下）接近 0，之后随能量上升出现明显吸收峰。
