---
title: "ABACUS 态密度（DOS/PDOS）计算教程"
...
### Q5：NSCF 找不到电荷密度文件（常见错误）

**症状：**
```
WARNING: "init_chg" is enabled but ABACUS failed to read charge density from file.
Can't open file OUT.ABACUS//ABACUS-CHARGE-DENSITY.restart
```

**可能原因：**
电荷密度文件不在正确位置。NSCF 计算会在 `OUT.ABACUS/` 目录下查找电荷密度文件。

**解决方法：**

**方法1：** 在 SCF 计算目录直接运行 NSCF
```bash
# 在同一目录下，先运行 SCF，再运行 NSCF
cp INPUT_scf INPUT && cp KPT_scf KPT
mpirun -np 8 abacus    # SCF 计算

cp INPUT_nscf INPUT && cp KPT_nscf KPT
mpirun -np 8 abacus    # NSCF 计算（自动找到电荷密度）
```

**方法2：** 手动复制电荷密度文件
```bash
# 如果在不同目录，需要复制电荷密度文件
mkdir -p nscf_dir/OUT.ABACUS
cp scf_dir/OUT.ABACUS/SPIN1_CHG nscf_dir/OUT.ABACUS/
cp scf_dir/OUT.ABACUS/ABACUS-CHARGE-DENSITY.restart nscf_dir/OUT.ABACUS/
cd nscf_dir && mpirun -np 8 abacus
```

**方法3：** 使用 read_file_dir 参数
```
# INPUT 文件中添加
read_file_dir    /path/to/scf/OUT.ABACUS/
```
...
**症状：**
- 计算的带隙明显小于实验值
- 例如 MgO：计算 4-5 eV，实验 7.8 eV

**原因：**
PBE 泛函系统性低估带隙，这是 DFT 的已知局限。
...
**解决方法：**
1. 接受 PBE 结果（定性分析）
2. 使用杂化泛函（HSE06）获得更准确带隙（计算量大）
3. 使用 GW 方法（更精确但计算量极大）

**注意：** DOS 的形状和轨道贡献分析不受影响，只是能量位置有偏移。
## 5.3 进阶技巧
...
对于铁磁或反铁磁体系，需要进行自旋极化计算（`nspin = 2`）。此时系统会分别计算自旋向上和自旋向下的电子态，输出两套独立的 DOS 数据。
...
在 STRU 文件的原子坐标行设置初始磁矩：
```
Fe  #label
2.2 #magnetism（初始磁矩，单位 μB）
1   #number of atoms
0.0  0.0  0.0  m  0  0  0
```
...
使用 abacustest 绘制磁性体系的 DOS：

```bash
abacustest model dos-pdos -j . --plot-type species
```

abacustest 会自动识别 `nspin = 2`，将自旋向下的 DOS 取负值，生成上下对称的磁性 DOS 图。

**典型特征：**
- 自旋向上和向下的 DOS 不对称
- 费米能级处两个自旋通道的 DOS 差值反映磁矩大小
- 交换劈裂导致的能量差反映磁性强度

### 5.4.4 参数建议

**smearing 参数：**
- 磁性金属（如 bcc Fe）：`smearing_sigma = 0.002` Ry
- 原因：磁性金属的费米面结构复杂，需要更小的展宽以准确描述
